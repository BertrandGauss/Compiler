...
void gen_graphic(normalNode* root){//画图 
	...
	while(!queue.empty()){
		...
		for(int i=0;;i++){
			...
			if(temp->next[i]->name=="ε") child="null";
	...
	while(pos!=nodeLength){
		...
		if(root->number==stack1[stack1.size()-1]){
			...
			if(Table[m][n][0]==0){//分析表为空 
				...
				cout<<endl;
				resultfile<<endl;
				if(nonTer[m].first=="factor")
					ErrorType="丢失操作数";
				else 
					ErrorType=nonTer[m].first+"错误"; 
				//cout<<nonTer[m].first<<" "<<Ter[n].first<<endl;
				SynAnalysis_ErrorProc(root,ErrorType);//错误处理 
				...
				resultfile<<"产生式：";
				for(int i=0;i<proNum;i++){
					if(proc[i][3]==Table[m][n][2]&&proc[i][1]==stack1[stack1.size()-1]){
						p->use=i;
						break;
					}
				}				 
				...
				for(int i=len-1;i>1;i--){				
					...
					ten->name = searchName(Table[m][n][i]);
					ten->pre = p;
					p->next[i-2]= ten;
